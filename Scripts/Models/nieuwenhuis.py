...
The attentional blink refers to the temporary impairment in perceiving the 2nd of 2 targets presented in close temporal proximity.
...
# The aim in this example is to reproduce Figure 3 from Nieuwenhuis et al. (2005) with Lag 2.
...
# Since the default function of the TransferMechansim is set to be Linear with default slope set to 1.0 and intercept set to 0.0,
# this TransferMechanism will take the input pattern as they are set to with no further manipulation.
...
decision_layer = LCA(size=3,                            # Number of units in input layer
                     ...
                     function=Logistic(bias=decbias),   # Set the Logistic function with bias = decbias
...
response_layer = LCA(size=2,                                        # Number of units in input layer
                     ...
                     function=Logistic(bias=respbias),    # Set the Logistic function with bias = decbias
                     noise=UniformToNormalDist(standard_dev = SD).function, # Set noise with seed generator compatible with MATLAB random seed generator 22 (rsg=22)
                     integrator_mode=True,                                         # Please see https://github.com/jonasrauber/randn-matlab-python for further documentation
                     name='RESPONSE LAYER')
...
LC = LCControlMechanism(integration_method="EULER",                 # We set the integration method to Euler like in the paper
                        ...
                        # decision_layer,
                        # ObjectiveMechanism(function=Linear, # Project the output of T1 and T2 but not the distraction unit of the decision layer to the LC with a linear function.
...
task.run(stim_list_dict, num_trials= 5)
