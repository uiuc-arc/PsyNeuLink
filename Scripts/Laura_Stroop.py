...
# row 0: input_'RED' to hidden_'RED', hidden_'GREEN'
# row 1: input_'GREEN' to hidden_'RED', hidden_'GREEN'
word_weights = pnl.MappingProjection(matrix=np.matrix([[2.63, -2.63],
                                                       [-2.63, 2.63]]),
                                     name='WORD_WEIGHTS')
...
cycles_mean = [np.mean(C_control),
                       np.mean(C_incongruent),
                       np.mean(C_congruent)]

cycles_x = np.array([0, 1, 2])
plt.plot(cycles_x[0:3], cycles_mean[0:3])
...
# plt.hist(results_CN_incongruent_trial[0])
# plt.show()


W_control = 12 * results_WR_control_trial[0][np.where(results_WR_control_trial[1]==1)]+ 206
W_congruent= 12 * results_WR_congruent_trial[0][np.where(results_WR_congruent_trial[1]==1)]+ 206
W_incongruent = 12 * results_WR_incongruent_trial[0][np.where(results_WR_incongruent_trial[1]==1)]+ 206


C_control = 12 * results_CN_control_trial[0][np.where(results_CN_control_trial[1]==1)]+ 206
C_congruent= 12 * results_CN_congruent_trial[0][np.where(results_CN_congruent_trial[1]==1)]+ 206
C_incongruent = 12 * results_CN_incongruent_trial[0][np.where(results_CN_incongruent_trial[1]==1)]+ 206
